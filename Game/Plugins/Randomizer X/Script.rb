...
module Randomizer
  ...
  def self.all_abilities
    keys = []
    GameData::Ability.each { |ability| keys.push(ability.id) }
    return keys
  end
  #-----------------------------------------------------------------------------
  #  command selection
  ...
  #-----------------------------------------------------------------------------
  #  randomizes abilities
  #-----------------------------------------------------------------------------
  def self.randomizeAbilities
	array = self.all_abilities
	abilitiesArr = []
  ...
  def self.randomizerSelection
    ...
    modifiers = [:ABILITIES, :TRAINERS, :ENCOUNTERS, :STATIC, :GIFTS, :ITEMS]
...
def randomizeSpecies(species, static = false, gift = false)
  ...
  if !pokemon.nil?
	...
	pokemon.ability = Randomizer.getRandomizedAbility(pokemon.species, pokemon.ability_id)
    pokemon.calc_stats
    pokemon.reset_moves
  end
...
def pbAddPokemon(*args)
  # randomizer
  args[0] = randomizeSpecies(args[0], false, true) if args.length > 2 && args[2] != 'starter'
  # gives Pokemon
...
#  AbilityChangeClass
#===============================================================================
class AbilityChangeClass
   def initialize(species, oldAbility, newAbility)
      @species = species
      @old_ability = oldAbility
